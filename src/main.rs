...
#[cfg(test)]
use packet_engine::{
    admit_udp_datagram, drop_unsupported_direct_udp, format_bytes, format_duration,
    handle_bridge_event, should_log_stale_bridge_event, spawn_agent_tcp_bridge,
    BridgeAdmissionStats, BridgeEventStats, DnsInflight, LocalDrainStats, RemoteBacklogPush,
    TunWriteStats, TunnelStats,
};
use packet_engine::{
    drain_local_bytes_to_bridges, ensure_bridges, expire_stale_flows, handle_bridge_event_into,
    prune_closed_flows, run_tunnel_loop, tun_ipv4_packet, write_packets_to_tun, RemoteBacklogs,
    PACKET_BUF_SIZE,
};
