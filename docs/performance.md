...
framed-agent `agent_send_credit_wait` and `agent_send_outbound_wait`,
...
forwarding, remote first-byte delay, flow duration/windowing, framed-agent
flow-control credit, framed-agent outbound queue pressure, supervisor event
pressure, or failed/reset flows. The trace is deliberately opt-in and does not
